...
/*=============socket creating==============*/ 
	...
	if (sockfd == -1) printf ("client socket error\n"); 
...
/*=========for username=====================*/ 
	bzero(buf,10); printf("\nENTER YOUR NAME::"); 
	fgets(buf,10,stdin); __fpurge(stdin); 
	buf[strlen(buf)-1]=':'; 
/*=============client connect to server============*/ 
	if(connect(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr))==-1) { printf("client connect error\n"); exit(0); } 
	else printf("%s connected to server\n",buf); 
	...
	pthread_join(thr2,NULL); pthread_join(thr1,NULL); 
	return 0; } 
/*main ends*/ 
/*======================================================================*/ 
	...
	if (signal(SIGINT,(void *)quitproc)==0) 
		if(signal(SIGTSTP, (void *)zzz)==0) 
			while(1) { 
				...
				if(n>0){ printf("\n%s ",buffer); bzero(buffer,MAXDATALEN); } }//while ends 
} 
...
//		printf("%s",buf); 
		...
		if(strlen(buffer)-1>sizeof(buffer)){ 
			...
			bzero(buffer,MAXDATALEN); __fpurge(stdin); } 
		n=send(sockfd,buffer,strlen(buffer),0); 
		if(strncmp(buffer,"quit",4)==0) exit(0); bzero(buffer,MAXDATALEN); }//while ends 
} 
/*======handling signals==========*/ 
