/* t1.c - code for server program that uses TCP/IP */ /****SERVER CODE****/ 
#include <stdio.h> 
#include <stdlib.h> 
...
/*Note: The port argument is optional. If no port is specified, * the server uses the default given by PORT.*/ 
...
{ 	int sockfd,new_fd; /*variables for socket*/ 
	...
	else portnum = PORT; //if port number not given as argument then using default port 
	printf("PORT NO.:\t%d\n",portnum); 
	...
	if(sockfd == -1){ printf("server- socket() error"); // debugging 
		exit(1); }
	...
	while(1){ 
		...
		if(recv(new_fd,username,sizeof(username),0)>0); 
			...
			a = a->next; /*=notify all clients about newly joining clients=*/ 
			a = h ; 
		do{ a = a->next; sf2 = a->port; 
			if(sf2!=new_fd) 
...
void *server(void * arguments){ 
	...
	addr a; /*=sending list of clients online=*/ 
	a =h ; 
	do{ a = a->next; sprintf( ubuf," %s is online\n",a->username ); send(ts_fd,ubuf,strlen(ubuf),0); } 
	while( a->next != NULL ); /*=start chatting=*/ 
	while(1){ bzero(buffer,256); 
	y=recv(ts_fd,buffer,MAXDATALEN,0); 
	if (y==0) goto jmp; /*=if a client quits=*/ 
	if ( strncmp( buffer, "quit", 4) == 0 ){ 
		...
		do{ 
			a = a->next; 
			sfd = a->port; 
			if(sfd == ts_fd) Delete( sfd, h ); 
			if(sfd != ts_fd) send(sfd,buffer,MAXDATALEN,0); }
		while ( a->next != NULL ); 
		Display( h ); 
		close(ts_fd); 
		free(msg); 
...
/*=sending message to all clients =*/ 
	...
	do{ 
		a = a->next; 
		sfd = a->port; 
		if(sfd != ts_fd) send(sfd,msg,msglen,0); } while( a->next != NULL ); 
	Display( h ); 
	bzero(msg,MAXDATALEN); }//end while 
	return 0; }// end server 
/*=====empties and deletes the list======*/ 
	head MakeEmpty( head h ) { if( h != NULL ) DeleteList( h ); 
		h = malloc( sizeof( struct Node ) ); 
		if( h == NULL ) printf( "Out of memory!" ); 
		h->next = NULL; 
		return h; } 
/*======delete list=======*/ 
void DeleteList( head h ) { 
	...
	a = h->next; 
	h->next = NULL; 
	while( a != NULL ) { Tmp = a->next; free( a ); a = Tmp; } } 
...
void Display( const head h ) { 
	addr a =h ; 
	if( h->next == NULL ) printf( "NO ONLINE CLIENTS\n" ); 
	else { 
		do { 
			a = a->next; 
			printf( "%d->%s \t", a->port,a->username ); } while( a->next != NULL ); printf( "\n" ); } } 
/*===========client deleted from list if client quits================*/ 
void Delete( int port, head h ){ 
	addr a, TmpCell; 
	a = h; 
	while( a->next != NULL && a->next->port != port ) a = a->next; 
	if( a->next != NULL ){ TmpCell = a->next; a->next = TmpCell->next; free( TmpCell ); } } 
/*======handling signals==========*/ 
...
void Quitall(){ 
	...
	if( h->next == NULL ) { 
		...
		do{ 
			...
			a = a->next; 
			sfd = a->port; 
			send(sfd,"server down",13,0); } while( a->next != NULL ); 
		printf("%d clients closed\n\n",i); } } 

void zzz(){ printf("\rDISPLAYING ONLINE CLIENTS\n\n"); Display(h); }
